...
async def perform_operation(
    ...
    file: UploadFile = File(
        default=None,
        description="Bytes if second operand is of type torch.Tensor or numpy.ndarray",
    ),
    x_oblv_user_name: str = Header(default=None),
    x_oblv_user_role: str = Header(default=None),
):
...
def publish_current_budget(
    ...
    if x_oblv_user_role != "domain":
        raise HTTPException(403, f"Path only available for domain role.")
    request = [
        x for x in router.publish_queries if x["request_id"] == input.publish_request_id
    ]
    ...
    for x in request[0]["owners"]:
        if x["owner"] == x_oblv_user_name:
            ...
            break
    background_tasks.add_task(check_all_budgets_received, input.publish_request_id)
    if flag:
        return "Success"
    else:
...
def publish_check_result_status(
    ...
    request = [
        x for x in router.publish_queries if x["request_id"] == publish_request_id
    ]
    if request.__len__() == 0:
        raise HTTPException(400, "Publish request id not found")
    ...
    for x in router.publish_queries:
        if x["request_id"] == publish_request_id:
            try:
                ...
                x[
                    "error"
                ] = f"Failed to prepare the result for this request due to exception: {e}"
